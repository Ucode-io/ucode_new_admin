...
const requestNoProject = axios.create({
  baseURL,
  timeout: 100000,
});
  ...
  if (
    ...
    return authService
      ...
      .then((res) => {
        ...
        return requestNoProject(originalRequest);
      })
      .catch((err) => {
        console.log(err);
        store.dispatch(authActions.logout());
...
requestNoProject.interceptors.request.use(
  (config) => {
...
requestNoProject.interceptors.response.use((response) => {
  customMessageHandler(response);
  return response.data.data;
}, errorHandler);

export default requestNoProject;
